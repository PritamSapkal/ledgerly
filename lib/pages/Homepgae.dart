...
class _HomepgaeState extends State<Homepgae> {
  final Box<expense_model> expenseBox = Hive.box<expense_model>('expenses_box');
  String? gretingtitle;
  String? sub;
  ...
  void removeExpense(expense_model expense) {
    final currentList = expenseBox.values.toList();
    final expenseIndex = currentList.indexOf(expense);
    setState(() {
  ...
  void openAddExpenseOverlay() {
    showModalBottomSheet(
      ...
      context: context,
      builder: (ctx) => AddNewExpense(addnewexpense: addExpense),
    );
  }
  ...
  Widget build(BuildContext context) {
    final List<expense_model> _registredexpenses =
        expenseBox.values.toList().cast<expense_model>()
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Lottie.asset(
                "lib/images/cycle_coin.json",
                height: screenHeight * 0.09,
                fit: BoxFit.contain,
                frameRate: FrameRate(3000),
                repeat: true,
              ),
            ),

            Text(
              'Your expense list is empty. Tap + to add one!',
              style: Theme.of(context).textTheme.headlineSmall!.copyWith(
                fontSize: (screnWidth * 0.038).roundToDouble().clamp(14, 20),
              ),
            ),
          ],
        ),
    ...
    if (_registredexpenses.isNotEmpty) {
      maincontext = Expanded(
        child: Container(
          child: Expense_List(
            expenseslist: _registredexpenses,
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.start,
          //_registredexpenses.isNotEmpty?MainAxisAlignment.start:MainAxisAlignment.center,
